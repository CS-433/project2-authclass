...
def feed_string(feed):
    ...
    Input :
        - feed : list of string tokens
    Ouput : 
        - feed transformed into one long string 
    '''
    print('Joining feed...')
    return ' '.join(feed)
...
def build_cohorts(n_native,n_nonnat,eng_native,eng_nonnat,seed):
    ...
    Input :
        - n_native : integer, number of native authors
        - n_nonnat : integer, number of non-native authors
        - eng_native : pandas dataframe, data of all native authors
        - eng_nonnat : pandas dataframe, data of al non-native authors
        - seed : integer, seed for randomness reproductibility
    Ouput :
        - cohort_native :  pandas dataframe, contains data of the 30 randomly selected native authors
        - cohort_nonnat :  pandas dataframe, contains data of the 30 randomly selected non-native authors
        - cohort_all : pandas dataframe, contains data of 15 of the 30 randomly selected native authors
                                              and data of 15 of the 30 randomly selected non-native authors
...
def find_features_param(dict_keys):
    ''' Returns the list of the names of the features to adjust to the config file parameters
    Input :
        - dict keys : python dictionnary keys
    Ouput :
        - list_keys : list of the keys that are names of features
    '''
...
def max_collection_size(var,X):
    ''' Returns the number of columns associated to a specific feature 
        ex: If var='letter_1gram' 
            And X contains the columns 'letter_1gram_1','letter_1gram_2',...,'letter_1gram_26'
...
def extract_features(cohort,config,filetag):
    ''' Extract and compute features from a dataset, split the dataset in test/train set 
    Input :
        - cohort  : pandas dataframe 
        - config  : python dictionnary, paramters of the features to extract
        - filetag : string, useful for saving things with meaningfull file names
    Ouput :
        - X_train : pandas dataframe, design matrix of train set
        - X_test  : pandas dataframe, design matrix of test set
        - y_train : pandas dataframe, labels of train set
        - y_test  : pandas dataframe, labels of test set 
    '''
    ...
    # Uncomment to save dataframes in files 
    #path = "../../Data/Test/"
    #X_train.to_parquet(path+"X_train_"+filetag)
    #X_test.to_parquet(path+"X_test_"+filetag)
    #y_train.to_parquet(path+"y_train_"+filetag)
    #y_test.to_parquet(path+"y_test_"+filetag)
...
def classify_with_input(filetag,config,X_train,X_test,y_train,y_test):     
    ''' Perform training/testing with the specific feature paramters
        Input: 
            - filetag : string, specific name to keep the info about what data is used
            - config  : python dict, contains all parameters of the classification
            - X_train : pandas dataframe, corresponds to the maximal design matrix (maximal number of features) for the training set
            - X_test  : pandas dataframe, corresponds to the maximal design matrix for the test set
            - y_train : pandas dataframe, labels (authors) of train set
            - y_test  : pandas dataframe, labels (authors) of test set 
        Output : 
            - dict_result : python dictionnary, contains accuracy, f1 score and infos about the model
    '''
    # Drop features (columns) according to the configuration passed as argument
    X_train_ = X_train
    X_test_ = X_test
    ...
    # Perform classification (train model of training data, test on testing data)
...
def classify(filetag, kernel,config,X_train,X_test,y_train,y_test):
    ''' Given the design matrix of the train set and its labels,an svm linear model is trained 
        Then, it is tested with the test set data. The accuracy and f1 score of the model are 
        computed and stored in a dictionnary
    Input :
        - filetag : string, to specify info in the resulting dictionnary 
        - kernel : string, kernel to use in the svm (in the end, we're always using 'linear')
        - config : python dictionnary containing seed info and hyperparamter C
        - X_train : pandas dataframe, design matrix of train set
        - X_test  : pandas dataframe, design matrix of test set
        - y_train : pandas dataframe, labels of train set
        - y_test  : pandas dataframe, labels of test set 
    Output :
        - dict_file : python dictionnary containing the accuracy and f1 score of the model, wrt the different cohorts in the train/test data
    '''
    # Set SVM parameters 
    C_svm = config['C_svm']
    if config['penalty']==2:
